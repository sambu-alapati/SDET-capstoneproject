@regression @registration-feature
Feature: User Registration
 
  @register @smoke @sanity @positive
  Scenario Outline: Register new user with valid details
    ...
    Examples:
      ...
      | newdemo123 | Test@123 | Test@123        | Naga      | Sai      | nagasai@gmail.com | 9876543210 | Whitefield | ITPL     | Bangalore | Karnataka | 560066 | India   | english  | FISH     |
 
  @validation @negative @mandatory-fields
  ...
  Scenario Outline: Verify existing user ID message 
    ...
    And User selects Favourite Category "<category>"
    And User clicks Save Account Information button
    Then User should see the existing user ID message "This user ID is already in use"
